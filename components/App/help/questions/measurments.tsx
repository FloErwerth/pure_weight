...
export const getMeasurementQuestions = (
    ...
    {
        title: language === "de" ? "Erstellung einer Messung" : "Creation of a measurement",
        answer:
            language === "de" ? (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            ...
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 4)}>Mehr dazu</InlinePressable>. <NewLine />
                            Wähle auch eine <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 2)}>Einheit</InlinePressable> und ein{" "}
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 3)}>Datum</InlinePressable> für deine Messung aus. <NewLine />
                            Nachdem Du deine Messung erstellt hast, drücke auf &quot;Messung erstellen&quot;. Deine Messung wird nun in der Übersicht deiner{" "}
                            <InlinePressable handlePress={handleNavigateToMeasurements}>Messungen</InlinePressable> angezeigt. <NewLine />
                        </AnswerText>
                    </ThemedView>
                ...
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            ...
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 4)}>Learn more</InlinePressable>. <NewLine />
                            Also select a <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 2)}>unit</InlinePressable> and a{" "}
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MEASUREMENTS, 3)}>date</InlinePressable> for your measurement. <NewLine />
                            ...
                            <InlinePressable handlePress={handleNavigateToMeasurements}>measurements</InlinePressable>. <NewLine />
                        </AnswerText>
                    </ThemedView>
                    <ThemedPressable safeBottom onPress={handleCreateNewMeasurement} center padding round>
                        <Text style={{ fontSize: 16 }}>Press here to create a measurement</Text>
                    </ThemedPressable>
                </HelpAnswer>
            ),
        snapPoints: ["85%"],
        ...
        answer:
            language === "de" ? (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            ...
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MISCELLANEOUS, 0)}>Mehr dazu</InlinePressable>.
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ) : (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            If you want to delete a measurement, <InlinePressable handlePress={handleNavigateToMeasurements}>navigate to the measurements</InlinePressable> and swipe the measurement to
                            be deleted to the left. Your measurement will now be deleted. <NewLine />
                            The deletion can be undone by pressing the button in the message at the bottom of the screen.{" "}
                            <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.MISCELLANEOUS, 0)}>Learn more</InlinePressable>.
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ),
        ...
        answer:
            language === "de" ? (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            ...
                            Drückst Du erneut auf &quot;Messung hinzufügen&quot;, so wird die vorhandene Messung durch die neue Messung ersetzt. <NewLine />
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ) : (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            There can only be one measurement per day. If you select a date on which a measurement already exists, you will see a warning when you first try to create your measurement.{" "}
                            <NewLine />
                            If you press &quot;Add measurement&quot; again, the existing measurement will be replaced by the new measurement. <NewLine />
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ),
        ...
        answer:
            language === "de" ? (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            Diese Option gibt an ob es besser ist, wenn der Wert einer Messung steigt oder fällt. <NewLine /> Dies hat nur Auswirkungen auf die Anzeige der Prognose in einer Messung,
                            welche in der <InlinePressable handlePress={handleNavigateToMeasurements}>Übersicht deiner Messungen</InlinePressable> angezeigt wird.
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ) : (
                <HelpAnswer>
                    <ThemedView ghost stretch style={{ gap: 20 }}>
                        <AnswerText>
                            This option indicates whether it is better if the value of a measurement increases or decreases. <NewLine /> This only affects the display of the forecast in a measurement,
                            which is displayed in the <InlinePressable handlePress={handleNavigateToMeasurements}>overview of your measurements</InlinePressable>.
                        </AnswerText>
                    </ThemedView>
                </HelpAnswer>
            ),
