            ...
            answer:
                language === "de" ? (
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                Nachdem Du ein <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 0)}>Workout gestartet</InlinePressable> hast erscheint die erste Übung des
                                Workouts. <NewLine />
                                ...
                                Ist es in den <InlinePressable handlePress={() => navigateToSettings(1)}>Workout Einstellungen</InlinePressable> unter &quot;Stoppuhr&quot; aktiviert, wird die Stoppuhr
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                After you have <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 0)}>started a workout</InlinePressable>, the first exercise of the workout
                                appears.
                                <NewLine />
                                The current set is highlighted by a {colorOfBackground} background. <NewLine /> To finish a set, press the{" "}
                                <ThemedMaterialCommunityIcons ghost name="check-bold" size={24} /> symbol. <NewLine /> If a set is marked as finished, the next active set is automatically selected and
                                the <ThemedMaterialCommunityIcons ghost name="check-bold" size={24} /> of the previous set turns green. <NewLine />
                                If the entry of the previous set is not correct, you can simply change the data. <NewLine />
                                If it is activated in the <InlinePressable handlePress={() => navigateToSettings(1)}>workout settings</InlinePressable> under &quot;Stopwatch&quot;, the stopwatch will
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                <NewLine />
                                Sofern Du es in den <InlinePressable handlePress={() => navigateToSettings(0)}>Allgemeinen Einstellungen</InlinePressable> unter &quot;Stoppuhr&quot; aktiviert hast,
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 1)}>all sets of an exercise</InlinePressable> and have activated it under &quot;Switch to
                                next exercise&quot; in the <InlinePressable handlePress={() => navigateToSettings(1)}>workout settings</InlinePressable>, the active exercise will automatically be
                                changed.
                                <NewLine />
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.EXERCISES, 1)}>Änderst Du die Übung</InlinePressable> während eines Trainings, so werden die leeren
                                Felder neu ausgefüllt, aber die bereits ausgefüllten Felder bleiben unverändert.
                            </AnswerText>
                        </ThemedView>
                    </HelpAnswer>
                ) : (
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.EXERCISES, 1)}>If you change the exercise</InlinePressable> during a training, the empty fields will
                                be filled in again, but the already filled fields will remain unchanged.
                            </AnswerText>
                        </ThemedView>
                    </HelpAnswer>
                ),
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                -Symbol neben den <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 5)}>Metadaten</InlinePressable> einer Übung geklickt wird. <NewLine />
            ...
            answer:
                language === "de" ? (
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                Hast Du die ausgewählte Übung in einem <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.WORKOUTS, 3)}>vorherigen Workout</InlinePressable> bereits
                                einmal absolviert, dann werden die Daten der letzten Durchführung unterhalb der aktuellen Übung angezeigt. <NewLine />
                                ...
                                Ähnlich wie bei dem <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 1)}>Durchführen eines Satzes</InlinePressable> wird in der Anzeige
                                der vorherigen Übung der aktuelle Satz mit einem {colorOfBackground} Hintergrund hervorgehoben. <NewLine />
                    ...
                    <HelpAnswer>
                        <ThemedView ghost stretch style={{ gap: 20 }}>
                            <AnswerText>
                                ...
                                <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.WORKOUTS, 3)}>previous workout</InlinePressable>, the data of the last execution will be displayed
                                below the current exercise. <NewLine />
                                ...
                                Similar to the <InlinePressable handlePress={() => handleSelectFromAnswer(SECTIONS.TRAININGS, 1)}>execution of a set</InlinePressable>, the current set is highlighted
                                with a {colorOfBackground} background in the display of the previous exercise. <NewLine />
