...
export const PreviousWorkout = ({ exerciseId, exerciseType }: PreviousTrainingProps) => {
	...
	const previousTrainingTitle = useMemo(
		() => previousWorkout?.date && t(TranslationKeys.PREVIOUS_TRAINING_NOTE_TITLE).concat(previousWorkout.date),
		[previousWorkout?.date, t],
	);
	if (!previousWorkout) {
		return null;
	}
