...
export const getTrainedWorkoutId = createSelector(
    [getTrainedWorkout],
    (trainedWorkout) => trainedWorkout?.workoutId,
);
...
export const getSearchedWorkout = createSelector(
    [getWorkoutState],
    (state) => state.searchedWorkout,
);
export const getPostWorkoutWorkoutId = createSelector(
    [getWorkoutState],
    (state) => state.postWorkoutWorkoutId,
);
...
export const getTrainedWorkoutExercises = createSelector(
    ...
    (trainedWorkout, workouts) => {
        ...
        return workouts.find((workout) => workout.workoutId === trainedWorkout.workoutId)
            ?.exercises;
    },
);
...
export const getExerciseById = createSelector(
    [getTrainedWorkout, (_, exerciseId: ExerciseId) => exerciseId, getWorkouts],
    (trainedWorkout, exerciseId, workouts) => {
...
export const getHasWeightInTimeBasedExercise = createSelector(
    ...
    (workouts, trainedWorkout, exerciseId) => {
        const workout = workouts.find((workout) => workout.workoutId === trainedWorkout?.workoutId);
        const trainedExercise = workout?.exercises.find(
            (exercise) => exercise.exerciseId === exerciseId,
        );
        const exercise = workout?.exercises.find(
            (exercise) => exercise.exerciseId === trainedExercise?.exerciseId,
        );
...
export const getIsEditedWorkout = createSelector(
    [getEditedWorkout],
    (editedWorkout) => !editedWorkout?.isNew,
);
...
export const getIsUsedInPausedWorkout = createSelector(
    [getWorkouts, getEditedExercise, getTrainedWorkout],
    (workouts, editedExercise, trainedWorkout) => {
...
export const getLatestWorkoutDateDisplay = createSelector(
    [getSortedDoneWorkout, getLanguage],
    (dates, language) => {
        return (
            getSinceDate(dates?.[dates?.length - 1], language ?? "de") ?? ("1970-01-01" as IsoDate)
        );
    },
);
...
export const getSortedWorkouts = createSelector(
    [getWorkouts, getWorkoutSorting],
    (workouts, sorting) => {
        return sortWorkouts(workouts, sorting);
    },
);
export const getNumberOfWorkouts = createSelector([getWorkouts], (workouts) => workouts.length);
export const getEditedWorkoutName = createSelector(
    [getEditedWorkout],
    (editedWorkout) => editedWorkout?.workout?.name,
);
    ...
    slicedDoneWorkouts?.forEach(({ isoDate, doneExercises }) => {
        if (doneExercises) {
            doneExercises.forEach(({ sets, originalExerciseId, type }) => {
                const originalName = workout?.exercises.find(
...
export const getWorkoutsByMonth = createSelector(
    ...
    (editedWorkout, searchedMonth) => {
        ...
        return workout?.doneWorkouts?.filter(
            ({ isoDate }) => getMonthYearLabel(isoDate) === searchedMonth,
        );
    },
);
export const getHasHistory = createSelector(
    [getWorkouts, (_, workoutId: WorkoutId) => workoutId],
    (workouts, workoutId) => {
        const doneWorkouts = workouts?.find((workout) => workout?.workoutId === workoutId)
            ?.doneWorkouts;
...
export const getPreviousWorkout = createSelector(
    [getTrainedWorkoutId, getLanguage, (_, __, exerciseId: ExerciseId) => exerciseId, getWorkouts],
    (trainedWorkoutId, language, exerciseId, workouts) => {
        const doneWorkouts = workouts.find((workout) => workout.workoutId === trainedWorkoutId)
            ?.doneWorkouts;
...
const getSumOfSets = (type: ExerciseType, sets?: ExerciseSets) => {
    ...
    return sets?.reduce(
        (sum, set) => sum + parseFloat(set?.reps ?? "0") * parseFloat(set?.weight ?? "0"),
        0,
    );
};
    ...
    (workouts, workoutId) => {
        ...
        if (latestWorkout) {
            latestWorkout.doneExercises?.forEach(({ originalExerciseId, sets, type }) => {
                ...
                const fittingExerciseBefore =
                    fittingExercisesBefore?.[fittingExercisesBefore.length - 1];
                ...
                if (fittingExerciseBefore) {
                    ...
                    if (
                        foundCompareables.find(
                            (compareable) => compareable.current.name === originalExercise?.name,
                        )
                    ) {
                        return;
                    }
...
export const getExerciseDone = createSelector(
    [getTrainedWorkout, getWorkouts],
    (trainedWorkout, workouts) => {
        ...
        return (
            trainedWorkout.exerciseData[exerciseIndex].sets.length === sets &&
            trainedWorkout.exerciseData[exerciseIndex].sets.every(({ confirmed }) =>
                Boolean(confirmed),
            )
        );
    },
...
export const getPauseTime = createSelector(
    [getTrainedWorkout, getWorkouts],
    (trainedWorkout, workouts) => {
        const exercise = workouts.find((workout) => workout.workoutId === trainedWorkout?.workoutId)
            ?.exercises[trainedWorkout?.activeExerciseIndex ?? -1];
        if (
            ...
            return (
                (parseFloat(exercise.pauseMinutes || "0") * 60 +
                    parseFloat(exercise.pauseSeconds || "0")) *
                1000
            );
        }
...
export const getIsDoneWithTraining = createSelector(
    [getTrainedWorkout, getWorkouts],
    (trainedWorkout, workouts) => {
        const workout = workouts.find((workout) => workout.workoutId === trainedWorkout?.workoutId);
        if (
            !workout ||
            trainedWorkout?.exerciseData.length === 0 ||
            trainedWorkout?.exerciseData.some((data) => data.sets.length === 0)
        ) {
            return false;
        }
...
export const getActiveSetIndex = createSelector(
    ...
    (trainedWorkout, exerciseId) => {
        return trainedWorkout?.exerciseData.find((exercise) => exercise.exerciseId === exerciseId)
            ?.activeSetIndex;
    },
);
...
export const getSetData = createSelector(
    [
        getTrainedWorkout,
        (_, setIndex: number) => setIndex,
        (_, __, exerciseId: ExerciseId) => exerciseId,
        getWorkouts,
    ],
...
export const getDoneExerciseById = createSelector(
    [
        getEditedWorkout,
        (_, doneWorkoutId: WorkoutId) => doneWorkoutId,
        (_, __, doneExerciseId) => doneExerciseId,
    ],
    (editedWorkout, doneWorkoutId, doneExerciseId) => {
        const doneWorkout = editedWorkout?.workout?.doneWorkouts.find(
            (doneWorkout) => doneWorkout.doneWorkoutId === doneWorkoutId,
        );
        return doneWorkout?.doneExercises?.find(
            (doneExercise) => doneExercise.doneExerciseId === doneExerciseId,
        );
    },
);

export const getPostWorkoutWorkout = createSelector(
    [getWorkouts, getPostWorkoutWorkoutId],
    (workouts, postWorkout) => {
...
export const getWorkoutStats = createSelector([getWorkouts], (workouts) => {
    return workouts.reduce(
        (stats, currentWorkout) => {
            ...
            stats.get(currentWorkout.workoutId)!.totalTimes = currentWorkout.doneWorkouts.length;
            ...
            const values = currentWorkout.doneWorkouts.reduce(
                ...
                { totalDuration: 0, totalSets: 0, totalReps: 0 } as {
            ...
            stats.get(currentWorkout.workoutId)!.totalDuration = values.totalDuration;
            stats.get(currentWorkout.workoutId)!.totalSets = values.totalSets;
            stats.get(currentWorkout.workoutId)!.totalReps = values.totalReps;
...
export const getOverallStats = createSelector(
    [({ settingsState }: AppState) => settingsState, getWorkouts, getWorkoutStats],
    ({ unitSystem }, workouts, stats) => {
        ...
        const overallStats = Array.from(stats.values()).reduce(
            (overallStats: OverallStats, currentStats) => {
                ...
                overallStats.totalTimes.value += currentStats.totalTimes;
                overallStats.totalSets.value += currentStats.totalSets;
                overallStats.totalReps.value += currentStats.totalReps;
                overallStats.totalDuration.value += currentStats.totalDuration;
                return overallStats;
            },
            {
                totalNumberOfWorkouts: {
        ...
        const { value, unit: durationUnit } = getDurationInSecondsMinutesOrHours(
            overallStats.totalDuration.value.toString(),
        );
