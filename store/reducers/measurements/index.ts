...
export const mutateEditedMeasurement = createAction<
...
>("save_inspected_measurement");
export const saveMeasurementDataPoint = createAction<{ datapoint?: MeasurementDataPoint; index?: number }, "save_measurement_data_point">(
    "save_measurement_data_point",
);
export const deleteMeasurementDataPoint = createAction<{ index?: number }, "delete_measurement_data_point">(
    "delete_measurement_data_point",
);
...
export const setSearchedMeasurements = createAction<string>("measurement_set_searched");
...
export const measurementReducer = createReducer<MeasurementState>({ measurements: [], sorting: "A_Z" }, (builder) => {
    builder
        ...
        .addCase(saveMeasurementDataPoint, (state, action) => {
            ...
            if (editedMeasurement && action.payload.datapoint && action.payload.index !== undefined) {
                const index = state.measurements.findIndex(
                    (measurement) => measurement.measurementId === editedMeasurement.measurement?.measurementId,
                );
        ...
        .addCase(saveEditedMeasurement, (state, action) => {
            const editedMeasurement = state.editedMeasurement;
            const newMeasurement = editedMeasurement?.measurement;
            const measurementsIndex = state.measurements.findIndex(
                (measurement) => measurement.measurementId === newMeasurement?.measurementId,
            );
        ...
        .addCase(mutateEditedMeasurement, (state, action) => {
            if (state.editedMeasurement && state.editedMeasurement.measurement) {
                state.editedMeasurement = {
                    ...state.editedMeasurement,
                    measurement: { ...state.editedMeasurement.measurement, [action.payload.key]: action.payload.value },
                };
            }
        })
        ...
        .addCase(recoverMeasurement, (state) => {
            if (state.deletedMeasurement?.measurement !== undefined) {
                ...
                newMeasurements.splice(
                    state.measurements.findIndex(
                        (measurement) => measurement.measurementId === state.deletedMeasurement?.measurement.measurementId,
                    ),
                    0,
        ...
        .addCase(recoverMeasurementDataPoint, (state) => {
            if (state.deletedMeasurementDataPoint && state.editedMeasurement) {
                state.editedMeasurement.measurement?.data.splice(
                    state.deletedMeasurementDataPoint.index,
                    0,
                    state.deletedMeasurementDataPoint?.dataPoint,
                );
            }
        });
});
